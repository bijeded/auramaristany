...
- **Llama a una función de Postgres con un cast local `// keep:`; NO pueble `Database["public"]["Functions"]` en `types.ts`.** Con `Functions` vacío supabase-js resuelve los embeds de forma laxa; en cuanto se puebla pasa a resolverlos contra `Relationships`, que este proyecto mantiene a `[]` a mano — y entonces fallan en `tsc` **todos** los selects con join del repo, no sólo el nuevo. El cast va sobre el **cliente**, nunca sobre el método: sacar `supabase.rpc` a una variable lo desliga de su receptor y dentro del SDK `rpc` lee `this.rest` → `Cannot read properties of undefined`. Compila, pasa lint, pasa los tests (un fake con arrow function no mira `this`) y da 500 en el navegador; un fake que valga tiene que ser función normal y comprobar su receptor. Corolario: **una función/columna creada en el mismo cambio no se mergea hasta que la migración esté aplicada y verificada contra la base real**, y el PR dice cuándo se aplicó; ni `tsc`, ni lint, ni los tests (que mockean el cliente), ni el build hablan con la base — misma familia que el `block_type` sin su `CHECK` y que los embeds de L2b (lección de D18).
- **Una unión de status/enum más estrecha que el `CHECK` de la base es una mentira que revienta en el render.** El `CHECK` de `subscriptions.status` (migración 017) admite **nueve** valores a propósito —`handleSubscriptionUpdated` espeja el de Stripe tal cual, y modelarlos es lo que evita que la escritura se rechace—, pero `SubStatus` declaraba **seis**. Dos consecuencias, y la peor no es la que se ve: `STATUS_BADGE` era un `Record<>` tecleado con esa unión estrecha e indexado **sin salida**, así que un `paused` devolvía `undefined` y `badge.label` tiraba el render entero — no esa fila, **el listado completo de clientes**, con sólo que Aura pausara una suscripción desde el dashboard de Stripe. Y donde el tipo estorbaba, alguien lo había tapado con un cast `// keep:` que se justificaba diciendo que era "más estrecho": el cast *era* el defecto, documentado. Reglas: (1) la unión de un campo con `CHECK` se **deriva** del tipo de `types.ts`, no se recopia — un alias, para que no puedan separarse sin que `tsc` lo note; (2) todo mapa indexado por ese campo se teclea `Record<string, …>` **con fallback** (`?? {…}`), porque el `CHECK` puede ensancharse otra vez sin que nadie se acuerde de la pantalla; (3) dos mapas del mismo campo en dos archivos son una sola tabla copiada, y les faltarán exactamente los mismos valores — se unifican. Ni `tsc` ni el lint ven este agujero: el tipo estrecho hace *imposible de escribir* la fila que la base sí guarda, así que el caso ni se puede testear hasta arreglar el tipo.
- **Al retirar o renombrar un símbolo exportado, actualiza en el MISMO cambio todo `docs/adr/*.md` que lo nombre.** Un ADR es el registro duradero que un lector futuro consulta antes que el código, así que uno desactualizado sobrevive al símbolo y manda a la gente a buscar algo que ya no existe — la misma enfermedad que se estaba curando, un directorio más allá. Ni `tsc` ni el lint miran Markdown (lección de D18: el ADR 0003 siguió nombrando `subscriptionGrantsPortalShell` hasta que lo cazó la revisión).
