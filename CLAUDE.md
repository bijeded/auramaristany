...
- **Una unión de status/enum nunca es más estrecha que el `CHECK` de la base.** (1) Se **deriva** del tipo de `types.ts` con un alias, no se recopia. (2) Todo mapa indexado por ese campo se teclea `Record<string, …>` **con fallback** (`?? {…}`). (3) Dos mapas del mismo campo en dos archivos son una tabla copiada: se unifican. Por qué: el `CHECK` de `subscriptions.status` (migración 017) admite **nueve** valores a propósito —`handleSubscriptionUpdated` espeja el de Stripe tal cual— y `SubStatus` declaraba **seis**. `STATUS_BADGE` se indexaba sin salida, así que un `paused` devolvía `undefined` y `badge.label` tiraba el render entero: no esa fila, **el listado completo de clientes**, con sólo que Aura pausara una suscripción desde el dashboard de Stripe. El CSV, con el mismo agujero, exportaba la columna Estado vacía. Donde el tipo estorbaba estaba tapado con un cast `// keep:` justificado por ser "más estrecho": el cast *era* el defecto, documentado. Y el tipo estrecho **esconde defectos de lógica, no sólo de render**: al ensancharlo apareció que `canDeleteClient` contaba `incomplete_expired` como suscripción viva, así que pedía "cancélala en Stripe" cuando no hay nada que cancelar y esa cliente no se podía borrar jamás — el mismo defecto que L2c ya había arreglado para `completed`. Ni `tsc` ni el lint lo ven, y **el caso no se puede ni testear hasta arreglar el tipo**: la unión estrecha hace imposible de escribir la fila que la base sí guarda.
