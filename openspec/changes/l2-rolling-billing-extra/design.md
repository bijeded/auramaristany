...
**Amended 2026-07-28 (second review pass).** Moving the status write in time re-opened the hole this decision exists to close, one month earlier: during the final paid month the row is `active` + `cancel_at_period_end = true`, which is exactly the shape of A9's grace window, so the page would offer "Reactivar" — clearing the scheduled cancellation and billing a month 7 that does not exist. The window is now a state of its own, keyed on `completed_at`: **scheduled to complete** (last paid month, still training, no actions offered) and **completed** (period over, graduated tier, CTA). The reactivation server action refuses a subscription carrying `completed_at` too — hiding a button is presentation, not enforcement.
