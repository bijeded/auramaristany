...
- [x] 4.2 On the Preview URL at ~375px (non-destructive, using an existing demo client): check `/portal/today` on an exercise with weight, and Desempeño with Peso selected. The toggles look identical and both animate. Headings are in Oswald and body text in Hind on the portal, admin and login pages.
- [x] 4.3 Remove D29, D32 and D33 from `BACKLOG.md`.
- [x] 4.4 Open PR `chore: unify weight toggle, font tokens and survey created_at type (D29, D32, D33)`. Silent-defect flag: **yes, `types.ts` of a table with a DB column constraint mismatch (D32)**. It isn't an enum, money or a migration.
