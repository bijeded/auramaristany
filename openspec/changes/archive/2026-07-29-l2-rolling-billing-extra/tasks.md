...
- [x] 7.5 Smoke that an Extra subscription passes month 6 without completing and keeps billing. — **Verificado en vivo** con un reloj de prueba sobre una Extra Avanzado real (modo test), colocada en su mes 5 y avanzada dos periodos contra el webhook de producción: mes 6 → `months_elapsed = 6`, `completed_at` null, `cancel_at_period_end` false, sin cancelación programada en Stripe; mes 7 → `months_elapsed = 7`, sigue `active`, tercera factura cobrada y registrada, y el contenido dio la vuelta (`content_ordinal` 1, `content_loops` 1) como manda el rolling de L2b. Es el mismo montaje que completó una CuarentaMás en el smoke de L2c, así que la diferencia la produce el `billing_model`, no el camino. Objetos de prueba borrados.
- [x] 7.6 Closes **D10** — **Verificado en vivo** sobre una suscripción real de Stripe (modo test, mes 2 de 6) en una cuenta con acceso al portal, que es lo que faltaba: los clientes demo llevan un `sub_seed_*` inventado y la acción de cancelar contesta 404. Recorrido completo cancelar → gracia → Reactivar → cancelar: la ventana de gracia muestra "Tu acceso termina el 29 de agosto de 2026" y "Reactivar mi plan", retira "Cancelar mi plan", y conserva el acceso al contenido. En la BD `status` se mantuvo `active` de punta a punta (nunca `canceled`) y `completed_at` siguió null, así que la gracia y la completion no se tocan — justo la confusión que costó las rondas 2 a 5 de revisión. Una sola fila de encuesta al final, prueba de que Reactivar sí borró la anterior. Objetos de prueba borrados.
