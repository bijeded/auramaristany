...
- [x] 6.3 After merge, `/opsx:archive` **alone** — it applies the spec deltas itself, rewriting the "Revenue by program" requirement in `openspec/specs/admin-dashboard-kpis/spec.md`. Then `openspec validate`, and delete the row in `BACKLOG.md` if one exists. ⚠ Corrected during execution: this task originally said `/opsx:sync` *then* `/opsx:archive`. Both apply the deltas, so the pair makes archive abort with `ADDED failed — already exists` and leaves a human deleting what sync wrote. `/opsx:sync` is for reconciling a delta that drifted **mid-flight**, not for closing a change.
