# long-text-wrapping Specification

## Purpose
How authored free text must wrap when it contains a token too long to fit its container — across the portal, the admin, and outgoing email — so that a pasted URL never escapes its card or makes a page scroll horizontally on mobile.
## Requirements
