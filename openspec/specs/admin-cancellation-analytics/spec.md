...
The admin dashboard's two churn charts (`/admin/dashboard`) — which subscriptions count as churn
and which count as "ever subscribed", why a graduation is excluded from the numerator while
remaining in the denominator, how each chart's percentage is defined, and why the two charts'
totals may legitimately disagree. Churn is a *historical* question about how a subscription ended,
which is not the question `deriveCancellationState` answers about a live one. See
`docs/adr/0006-churn-is-a-second-derivation-with-its-own-denominator.md`.
