# content-ladder-progression Specification

## Purpose
Where a client is in the content and where she goes next when she pays another month. The position is stored state on the subscription (`content_variant_id`, `content_ordinal`, `content_loops`) that advances exactly one step per newly recorded paid invoice — never a count derived from `months_elapsed`, which stays the billing and elapsed-time arbiter. Covers the advance rule and its branch order, per-invoice idempotency, entry at any rung, and how progress and repetition are shown to the client.
