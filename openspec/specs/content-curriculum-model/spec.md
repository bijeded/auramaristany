# content-curriculum-model Specification

## Purpose
How a variant's curriculum is ordered, and how the series to serve a subscription is resolved. Position lives on the variant-to-series mapping (`variant_series_map.ordinal`), never on the series itself, so each level numbers its months independently from 1 and two variants of one program can both hold a first month. Also declares the rung order between variants (`program_variants.ladder_next_variant_id`), which this capability stores but does not yet read.
## Requirements
