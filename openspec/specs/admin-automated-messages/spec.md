...
## Purpose
The admin surface for the automated message rules: editing each rule's copy and switching it on or off. The rule set is fixed — triggers live in code, so the screen edits rows, it does not create or delete them.
## Requirements
