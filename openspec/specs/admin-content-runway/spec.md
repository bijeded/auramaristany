# admin-content-runway Specification

## Purpose
Warning Aura which clients are about to run out of newly authored content, before a client finds out first. Covers both exhaustion shapes — a client approaching the end of the top rung who will begin repeating, and a client approaching the end of a rung whose declared next rung has no series authored — as one list, because both have the same cause and the same remedy.
