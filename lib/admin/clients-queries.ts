...
export async function getClientsList(): Promise<ClientListRow[]> {
  ...
  const { data } = await supabase
    .from("subscriptions")
    .select(
      "profile_id, status, completed_at, cancel_at_period_end, current_period_end, enrollment_date, created_at, profiles(full_name, email, phone, progress_logs(log_date)), program_variants!program_variant_id(name, price_mxn, programs(name))"
    );
