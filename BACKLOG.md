...
```
/opsx:propose "D26 — CANCELABLE_STATUSES duplication"   # when scope is already clear
...
| **D25** | Filter pills carry no `aria-pressed` | XS | Each pill is a plain `<button>` whose active state is CSS-only, so a screen reader gets no signal. ⚠ **Narrowed by PR #49:** the status pills became a `<select>`, which announces its own state, so this now applies only to the **program pills** in `ClientsTable` (and to `PaymentsTable`'s pills, see **D31**). One attribute: `aria-pressed={prog === f}`. |
