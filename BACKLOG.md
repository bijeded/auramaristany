...
| **D10** | Verify A9 cancellation end-to-end after demo refresh | S | **Still open after L2c.** Demo subs use fabricated `sub_seed_*` Stripe IDs → `cancelSubscription`/`reactivateSubscription` 404 in Stripe test mode ("No se pudo guardar. Intenta más tarde."). Not a code bug — *no demo client can exercise this at all*, which is why it keeps deferring. Confirmed again 2026-07-29: attempted during the L2c smoke and blocked for exactly this reason. Closing it needs a **real** Stripe test-mode subscription on a portal-loggable account (the L2c smoke showed this takes ~2 min to set up); then cancel → grace → Reactivar → confirm the survey row is written and deleted. Tie to **L4** / **L6**. |
...
| **D17** | Dashboard projects phantom renewal revenue | S | From the L2c fix review (PR #24). `computeRenewalsWithinDays` counts every row whose `current_period_end` falls in the horizon, and `getActiveSubscriptions` selects on `status = 'active'` alone — so a subscription in an A9 grace window, or in its final fixed-term month (both `active` with `cancel_at_period_end = true`), still adds to "Renuevan este mes" and to the projected amount. Aura is shown money that will not arrive. Fix is to carry `status`/`cancel_at_period_end` into `lib/admin/finance-queries.ts` + `finance-helpers.ts` and exclude ending subscriptions — reusing the same single derivation, not a fourth inline copy. **Same family as the bug #24 fixed**, one screen further along. |
| **D18** | `subscriptionGrantsPortalShell` is exported and tested but never called | S | From the L2c archive verification. `middleware.ts` and `app/portal/layout.tsx` use `PORTAL_SHELL_STATES` + `derivePortalTier` instead, so the named predicate the delta spec asked for exists but has no caller. Harmless today; an unused exported predicate next to a live one is how a future reader picks the wrong one. Either call it from `derivePortalTier` or delete it. |
