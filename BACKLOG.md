...
| **D19** | The exit survey cannot tell "Otro" from "prefiero no decir" | XS | The modal models "Prefiero no decir" as `reason = null` and `lib/portal/settingsActions.ts` inserts `reason ?? "otro"`, so declining is stored as an explicit "Otro". Harmless to the cancellation flow, but it blunts the one report Aura would read to learn *why* clients leave. Fix: a distinct value (e.g. `prefiero_no_decir`) in `CancellationReason` **plus the matching `CHECK` migration in the same change** — see the enum/CHECK rule in `CLAUDE.md`. |
...
1. **Now — unblocked cleanup, cheapest first:** `D6` · `D19` · `D20` · `D23` · `D25` (all XS, batchable into one or two changes) → `D15` (needed before `L8`).
