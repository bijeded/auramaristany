...
```
/opsx:propose "D14 — admin layout guard split"   # when scope is already clear
...
1. **Now — the real S/M debts:** `D14` (admin layout guard split) · `D28` (demo data cannot exercise any Stripe-touching flow — it blocks smoke-testing cancel/reactivate, and has now cost two unrunnable smoke cards).
